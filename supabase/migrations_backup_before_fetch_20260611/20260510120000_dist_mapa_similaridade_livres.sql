-- Mapa semantico dos tickets livres nao mantidos do Distribuidor.
-- Retorna apenas metadados e arestas de similaridade; nunca expoe embeddings ao cliente.

DROP FUNCTION IF EXISTS public.dist_obter_mapa_similaridade_livres(uuid, text, real, integer, integer);
...
AS $$
  WITH parametros AS (
    SELECT
      LEAST(0.98::real, GREATEST(0.50::real, COALESCE(p_min_similarity, 0.82)::real)) AS min_similarity,
...
$$;
COMMENT ON FUNCTION public.dist_obter_mapa_similaridade_livres(uuid, text, real, integer, integer)
IS 'Retorna grafo semantico dos tickets livres nao mantidos do Distribuidor, usando embeddings sem expor vetores ao cliente.';
GRANT EXECUTE ON FUNCTION public.dist_obter_mapa_similaridade_livres(uuid, text, real, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.dist_obter_mapa_similaridade_livres(uuid, text, real, integer, integer) TO service_role;
