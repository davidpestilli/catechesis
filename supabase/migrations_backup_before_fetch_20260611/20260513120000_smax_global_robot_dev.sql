-- =====================================================
-- SMAX Global Robot (Dev)
-- Confer linked SMAX tickets against free Distribuidor tickets
-- and attach confirmed free tickets to an active Global.
-- =====================================================

BEGIN;
INSERT INTO public.permissoes_objetos (codigo, nome, descricao, categoria, origem)
VALUES (
  'distribuidor.smax_robo_dev',
  'Robo SMAX Global (Dev)',
  'Card dev para extrair tickets vinculados a um Global no SMAX e anexar livres ao Global no Distribuidor.',
  'distribuidor',
  'src/pages/Home.tsx'
)
...
BEGIN
  ...
  candidatos AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE regexp_replace(COALESCE(ticket.numero_chamado, ''), '[^0-9]', '', 'g') = e.numero
      ORDER BY ticket.updated_at DESC NULLS LAST, ticket.created_at DESC NULLS LAST
      LIMIT 1
    ) t ON true
  ...
  SELECT
    ...
    CASE
      ...
      WHEN c.mantido_por IS NOT NULL THEN 'ocupado'
      ELSE 'livre'
    END AS motivo
  FROM candidatos c
...
END;
$$;
CREATE OR REPLACE FUNCTION public.smax_global_conferir_livres(
  p_equipe_id uuid,
  p_global_id uuid,
  p_numeros text[]
)
...
DECLARE
  ...
  v_total_informados integer := 0;
  v_total_validos integer := 0;
  v_total_livres integer := 0;
BEGIN
  SELECT count(*)::integer
    INTO v_total_informados
  FROM unnest(COALESCE(p_numeros, ARRAY[]::text[])) AS raw(numero)
  WHERE trim(COALESCE(raw.numero, '')) <> '';

  SELECT cg.id, cg.numero, cg.nome
    INTO v_global
  FROM public.chamados_globais cg
  WHERE cg.id = p_global_id;
  ...
  lista AS (
    SELECT
      COALESCE(jsonb_agg(to_jsonb(c) ORDER BY c.numero_pesquisado), '[]'::jsonb) AS tickets,
      count(*)::integer AS total_validos,
      count(*) FILTER (WHERE c.motivo = 'livre')::integer AS total_livres
    FROM classificados c
  ),
  motivos AS (
    SELECT COALESCE(jsonb_object_agg(m.motivo, m.total), '{}'::jsonb) AS por_motivo
    FROM (
      SELECT c.motivo, count(*)::integer AS total
      FROM classificados c
      GROUP BY c.motivo
    ) m
  )
  SELECT lista.tickets, lista.total_validos, lista.total_livres, motivos.por_motivo
    INTO v_tickets, v_total_validos, v_total_livres, v_por_motivo
  FROM lista CROSS JOIN motivos;

  RETURN jsonb_build_object(
    'global', jsonb_build_object(
      'id', v_global.id,
      'numero', v_global.numero,
      'nome', v_global.nome
    ),
    'total_informados', v_total_informados,
    'total_validos', COALESCE(v_total_validos, 0),
    'total_livres', COALESCE(v_total_livres, 0),
...
BEGIN
  ...
  candidatos AS (
    SELECT t.ticket_uuid, t.numero_chamado
    FROM entrada e
    JOIN LATERAL (
      SELECT ticket.id AS ticket_uuid, ticket.numero_chamado
      FROM public.tickets ticket
      WHERE regexp_replace(COALESCE(ticket.numero_chamado, ''), '[^0-9]', '', 'g') = e.numero
        AND ticket.gse = ANY(v_gses)
        AND ticket.status = 'aguardando'
        AND ticket.usuario_atual IS NULL
        AND COALESCE(ticket.suspenso, false) = false
        AND ticket.mantido_por IS NULL
        AND ticket.chamado_global_id IS NULL
        AND NOT EXISTS (
          SELECT 1 FROM public.tickets_globais tg WHERE tg.ticket_id = ticket.id
        )
      ORDER BY ticket.updated_at DESC NULLS LAST, ticket.created_at DESC NULLS LAST
      LIMIT 1
...
REVOKE ALL ON FUNCTION public.smax_global_conferir_livres(uuid, uuid, text[]) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.smax_global_conferir_livres(uuid, uuid, text[]) TO authenticated;
