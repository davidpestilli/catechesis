-- =====================================================
-- SOS: incluir tempo medio de atendimento (hoje/semana)
-- Formula: tempo_espera_origem -> finished_at
-- Janela: baseada em finished_at no timezone America/Sao_Paulo
-- =====================================================

DROP FUNCTION IF EXISTS public.sos_obter_metricas_atendidos(uuid);
...
RETURNS TABLE(
  ...
  tempo_medio_hoje_segundos bigint,
  tempo_medio_semana_segundos bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      EXTRACT(EPOCH FROM (t.finished_at - t.tempo_espera_origem))::bigint AS duracao_segundos,
      (t.finished_at AT TIME ZONE v_timezone) AS finished_local
    FROM public.tickets t
    WHERE t.sos = true
      AND t.finished_at IS NOT NULL
      AND t.tempo_espera_origem IS NOT NULL
      AND t.finished_at >= t.tempo_espera_origem
      AND t.gse = ANY(v_gses)
  )
  SELECT
    COUNT(*) FILTER (
      WHERE b.finished_local >= v_hoje_inicio
        AND b.finished_local < v_amanha_inicio
    )::bigint AS atendidos_hoje,
    COUNT(*) FILTER (
      WHERE b.finished_local >= v_semana_inicio
        AND b.finished_local < v_proxima_semana_inicio
    )::bigint AS atendidos_semana,
    COALESCE(
      ROUND(
        AVG(b.duracao_segundos) FILTER (
          WHERE b.finished_local >= v_hoje_inicio
            AND b.finished_local < v_amanha_inicio
        )
      )::bigint,
      0::bigint
    ) AS tempo_medio_hoje_segundos,
    COALESCE(
      ROUND(
        AVG(b.duracao_segundos) FILTER (
          WHERE b.finished_local >= v_semana_inicio
      ...
      0::bigint
    ) AS tempo_medio_semana_segundos
  FROM base b;
END;
$$;
...
COMMENT ON FUNCTION public.sos_obter_metricas_atendidos(uuid) IS
  'Retorna quantidade de tickets SOS atendidos e tempo medio (entrada->encerramento) hoje e na semana vigente (domingo-sabado) para a equipe informada.';
