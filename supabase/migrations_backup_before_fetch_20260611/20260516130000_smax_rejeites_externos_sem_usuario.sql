-- =====================================================
-- SMAX Rejeites - Externos e Sem Usuario
-- Inclui no snapshot tickets com especialista nao mapeado
-- ou sem Designado Especialista na grade do SMAX.
-- =====================================================

BEGIN;
ALTER TABLE public.smax_rejeites_snapshot
  ADD COLUMN IF NOT EXISTS smax_nome_original text,
  ADD COLUMN IF NOT EXISTS mapeado boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS grupo_rejeite text NOT NULL DEFAULT 'usuario_mapeado';
ALTER TABLE public.smax_rejeites_snapshot
  DROP CONSTRAINT IF EXISTS smax_rejeites_snapshot_grupo_rejeite_check;
ALTER TABLE public.smax_rejeites_snapshot
  ADD CONSTRAINT smax_rejeites_snapshot_grupo_rejeite_check
  CHECK (grupo_rejeite IN ('usuario_mapeado', 'externo_ou_sem_usuario'));
UPDATE public.smax_rejeites_snapshot
SET smax_nome_original = COALESCE(smax_nome_original, smax_nome),
    mapeado = COALESCE(mapeado, true),
    grupo_rejeite = COALESCE(grupo_rejeite, 'usuario_mapeado')
WHERE smax_nome_original IS NULL
   OR mapeado IS NULL
   OR grupo_rejeite IS NULL;
CREATE INDEX IF NOT EXISTS idx_smax_rejeites_snapshot_equipe_grupo_data
  ON public.smax_rejeites_snapshot(equipe_id, grupo_rejeite, ultima_atualizacao DESC);
ALTER TABLE public.smax_rejeites_snapshot_meta
  ADD COLUMN IF NOT EXISTS total_externo_sem_usuario integer NOT NULL DEFAULT 0;
