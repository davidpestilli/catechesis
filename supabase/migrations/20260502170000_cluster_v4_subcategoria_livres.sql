-- ============================================================
-- Migracao: Sistema Solar v4 - agrupamento por subcategoria + embedding
-- Data: 2026-05-02
--
-- Objetivo:
--   Aumentar a cobertura util do Sistema Solar agrupando apenas tickets
--   livres acionaveis e criando arestas vetoriais somente dentro da
--   mesma subcategoria. Isso evita mega-componentes causados por pontes
--   semanticas entre assuntos diferentes.
-- ============================================================
...
BEGIN
  ...
  DROP TABLE IF EXISTS _candidatos, _arestas, _uf, _componentes, _grupos,
    _grupos_refinados, _ref_frontier, _ref_candidatos, _ref_arestas,
    _ref_uf, _ref_componentes;
  ...
  IF v_n_avaliados = 0 THEN
    RETURN jsonb_build_object(
      ...
      'algoritmo', 'v4-subcategoria-embedding',
  ...
  LOOP
    ...
    INSERT INTO _ref_candidatos(ticket_id, embedding, categoria_equipe_id, subcategoria_gse_id, gse)
    SELECT c.ticket_id, c.embedding, c.categoria_equipe_id, c.subcategoria_gse_id, c.gse
    FROM _candidatos c
    ...
    WHERE a.subcategoria_gse_id IS NOT NULL
      AND (1 - (a.embedding <=> b.embedding)) >= v_ref_threshold;
  ...
  inserts AS (
    ...
    SELECT
      ...
      g.root,
      CASE
        WHEN g.refinado THEN 'v4-subcat-refine-max' || p_max_satelites::text
        ELSE 'v4-subcat-direct-max' || p_max_satelites::text
      END,
      g.threshold_usado,
      'pendente'
    FROM grupos g
...
COMMENT ON FUNCTION public.cluster_tickets_equipe(uuid, real, integer, integer, integer) IS
  'Agrupa tickets livres por embeddings dentro da mesma subcategoria. Desde v4, usa somente tickets acionaveis sem reserva para aumentar cobertura util e evitar mega-componentes por pontes semanticas.';
