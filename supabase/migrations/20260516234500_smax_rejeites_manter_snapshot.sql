-- =====================================================
-- SMAX Rejeites - Manter/Liberar Snapshot
-- Adiciona reserva por usuario aos tickets rejeitados e
-- RPCs para manter/liberar individualmente ou em lote.
-- =====================================================

BEGIN;
ALTER TABLE public.smax_rejeites_snapshot
  ADD COLUMN IF NOT EXISTS mantido_por uuid REFERENCES public.users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS mantido_at timestamptz;
CREATE INDEX IF NOT EXISTS idx_smax_rejeites_snapshot_equipe_mantido
  ON public.smax_rejeites_snapshot(equipe_id, mantido_por, mantido_at DESC);
...
BEGIN
  ...
  SELECT s.ticket_numero, s.mantido_por, s.mantido_at
...
END;
$$;
CREATE OR REPLACE FUNCTION public.smax_rejeites_manter(p_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF v_rejeite.id IS NOT NULL THEN
    RETURN jsonb_build_object(
      'success', true,
      'id', v_rejeite.id,
  ...
  SELECT s.id, s.ticket_numero, s.mantido_por, s.mantido_at, u.nome AS mantido_por_nome, u.email AS mantido_por_email
    INTO v_rejeite
  FROM public.smax_rejeites_snapshot s
  LEFT JOIN public.users u ON u.id = s.mantido_por
  WHERE s.id = p_id
    AND s.equipe_id = v_user_equipe_id;
  ...
  IF v_rejeite.mantido_por = v_user_id THEN
    RETURN jsonb_build_object(
      'success', true,
      'id', v_rejeite.id,
      'ticket_numero', v_rejeite.ticket_numero,
...
END;
$$;
CREATE OR REPLACE FUNCTION public.smax_rejeites_manter_lote(p_ids uuid[])
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
REVOKE ALL ON FUNCTION public.smax_rejeites_manter(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.smax_rejeites_liberar(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.smax_rejeites_manter_lote(uuid[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.smax_rejeites_liberar_lote(uuid[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.smax_rejeites_manter(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.smax_rejeites_liberar(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.smax_rejeites_manter_lote(uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.smax_rejeites_liberar_lote(uuid[]) TO authenticated;
