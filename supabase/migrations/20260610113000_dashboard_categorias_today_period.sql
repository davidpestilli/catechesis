-- Adiciona o período "today" ao Dashboard de Categorias.
-- O recorte usa o dia civil vigente em America/Sao_Paulo (00:00:00 a 23:59:59),
-- preservando os períodos móveis já existentes (24h/48h/72h/etc).
...
BEGIN
  ...
  CASE p_periodo
    WHEN 'today' THEN
      v_data_inicio := date_trunc('day', v_now);
  ...
  IF p_incluir_comparativo AND p_periodo != 'all' THEN
    ...
    CASE p_periodo
      WHEN 'today' THEN v_prev_data_inicio := v_prev_data_fim - INTERVAL '1 day';
...
CREATE OR REPLACE FUNCTION public.obter_categorias_temporais(
  ...
  IF v_usa_hierarquico THEN
    ...
    contagens AS (
      SELECT
        p.periodo_inicio,
        items.item_id,
        COUNT(b.tempo_espera_origem) AS total
      FROM periodos p
      CROSS JOIN (SELECT UNNEST(v_items_uuid) AS item_id) items
      LEFT JOIN base_data b
        ON b.tempo_espera_origem >= p.periodo_inicio
        AND b.tempo_espera_origem < p.periodo_inicio + v_intervalo::INTERVAL
        AND (
          CASE WHEN v_modo_subcategoria
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION public.obter_categorias_temporais(TEXT, UUID, TEXT, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.obter_categorias_temporais(TEXT, UUID, TEXT, UUID) TO service_role;
CREATE OR REPLACE FUNCTION public.obter_sunburst_categorias(
  p_equipe_id UUID DEFAULT NULL,
  p_periodo TEXT DEFAULT '7d',
  p_gse TEXT DEFAULT NULL
)
...
BEGIN
  ...
  IF v_usa_hierarquico THEN
    WITH base AS MATERIALIZED (
      SELECT ta.categoria_equipe_id, ta.subcategoria_gse_id, ta.categoria_gse_id
      FROM ticket_analises ta
      JOIN public.tickets t ON t.id = ta.ticket_id
      WHERE ta.categoria_equipe_id IS NOT NULL
        AND t.tempo_espera_origem >= v_data_inicio
    ...
    cat_totals AS (
      SELECT categoria_equipe_id, COUNT(*) AS total
      FROM base
      GROUP BY categoria_equipe_id
    ),
    subcat_totals AS (
      SELECT b.categoria_equipe_id, sg.slug AS subcat_slug, sg.nome AS subcat_nome, COUNT(*) AS total
      FROM base b
      JOIN subcategorias_gse sg ON sg.id = b.subcategoria_gse_id
      WHERE b.subcategoria_gse_id IS NOT NULL
      GROUP BY b.categoria_equipe_id, sg.slug, sg.nome
    ),
    subcat_matched AS (
      SELECT categoria_equipe_id, SUM(total) AS total_matched
      FROM subcat_totals
      GROUP BY categoria_equipe_id
    )
    SELECT jsonb_build_object(
      ...
      'children', COALESCE((
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'children', COALESCE(
              (
                SELECT jsonb_agg(item ORDER BY item_total DESC)
                FROM (
                  SELECT
                    jsonb_build_object(
                      'name', st.subcat_nome,
                    ...
                    st.total AS item_total
                  FROM subcat_totals st
                  WHERE st.categoria_equipe_id = ce.id
                  UNION ALL
                  SELECT
                    jsonb_build_object(
                      'name', 'Outros',
                      'slug', ce.slug || '-outros',
                      'cor', ce.cor_hex,
                      'value', ct.total - COALESCE(sm.total_matched, 0)
                    ) AS item,
                    ct.total - COALESCE(sm.total_matched, 0) AS item_total
                  FROM (SELECT 1) dummy
                  LEFT JOIN subcat_matched sm ON sm.categoria_equipe_id = ce.id
                  WHERE ct.total - COALESCE(sm.total_matched, 0) > 0
                ) combined
              ),
              jsonb_build_array(
                jsonb_build_object(
        ...
        FROM cat_totals ct
        JOIN categorias_equipe ce ON ce.id = ct.categoria_equipe_id
      ), '[]'::jsonb)
    )
    INTO v_resultado;
  ELSE
    WITH base AS MATERIALIZED (
      SELECT ta.categoria_slug, ta.subcategoria_slug
      FROM ticket_analises ta
      JOIN public.tickets t ON t.id = ta.ticket_id
      WHERE ta.categoria_slug IS NOT NULL
        AND ta.categoria_slug != 'indefinido'
    ...
    cat_totals AS (
      SELECT categoria_slug, COUNT(*) AS total
      FROM base
      GROUP BY categoria_slug
    ),
    subcat_totals AS (
      SELECT b.categoria_slug, b.subcategoria_slug, COUNT(*) AS total
      FROM base b
      JOIN subcategorias_chamado sc
        ON sc.slug = b.subcategoria_slug
        AND sc.categoria_slug = b.categoria_slug
        AND sc.ativo = TRUE
      WHERE b.subcategoria_slug IS NOT NULL
      GROUP BY b.categoria_slug, b.subcategoria_slug
    ),
    subcat_matched AS (
      SELECT categoria_slug, SUM(total) AS total_matched
      FROM subcat_totals
      GROUP BY categoria_slug
    )
    SELECT jsonb_build_object(
      ...
      'children', COALESCE((
            ...
            'children', COALESCE(
              (
                ...
                FROM (
                  ...
                  FROM subcat_totals st
                  JOIN subcategorias_chamado sc
                    ON sc.slug = st.subcategoria_slug
                    AND sc.categoria_slug = st.categoria_slug
                  WHERE st.categoria_slug = cc.slug
                  UNION ALL
                  SELECT
                    jsonb_build_object(
                      'name', 'Outros',
                    ...
                    ct.total - COALESCE(sm.total_matched, 0) AS item_total
                  FROM (SELECT 1) dummy
                  LEFT JOIN subcat_matched sm ON sm.categoria_slug = cc.slug
                  WHERE ct.total - COALESCE(sm.total_matched, 0) > 0
                ) combined
              ),
              jsonb_build_array(
        ...
        FROM cat_totals ct
        JOIN categorias_chamado cc ON cc.slug = ct.categoria_slug
      ), '[]'::jsonb)
    )
    INTO v_resultado;
  END IF;
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION public.obter_sunburst_categorias(UUID, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.obter_sunburst_categorias(UUID, TEXT, TEXT) TO service_role;
