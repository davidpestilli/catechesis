-- ============================================================
-- Migracao: Sistema Solar v6 - recall global + complete-link
-- Data: 2026-05-03
--
-- Objetivo:
--   Remover a trava dura de subcategoria da v5 e permitir que o
--   agrupamento considere todos os tickets livres entre si, desde que o
--   planeta final mantenha coesao interna total acima do threshold.
--
-- Estrategia:
--   1. Filtra apenas tickets livres acionaveis com descricao util.
--   2. Calcula compatibilidade global entre todos os pares candidatos.
--   3. Usa componentes conectados apenas para particionar o problema.
--   4. Dentro de cada componente, cria planetas por pair-seed +
--      complete-link: cada novo satelite precisa ser compativel com todos
--      os membros do planeta atual.
--   5. Reatribui orfaos apenas quando continuam compativeis com todo o
--      interior do planeta escolhido.
--   6. Grava medoid real como centroid_ticket_id.
-- ============================================================
...
BEGIN
  ...
  -- Mantido por compatibilidade com a API atual; a v6 nao usa top-k,
  ...
  -- Piso operacional da v6: qualquer planeta final precisa ter coesao total
  -- de no minimo 0.88; sliders acima disso endurecem o agrupamento.
  ...
  IF v_n_avaliados = 0 THEN
    RETURN jsonb_build_object(
      ...
      'algoritmo', 'v6-global-complete-pairseed',
    ...
    LOOP
      ...
      FROM (
        ...
        WHERE e.sim >= v_frontier.threshold_usado
          AND e.a_id = ANY(v_frontier.membros)
          AND e.b_id = ANY(v_frontier.membros)
      ...
      WHERE cand.common_count + 2 >= p_min_satelites
      ...
      IF v_group_count >= p_min_satelites THEN
        INSERT INTO _grupos(membros, tamanho, threshold_usado, origem_root)
        VALUES (v_group_members, v_group_count, v_frontier.threshold_usado, v_frontier.origem_root);
      ELSE
  ...
  inserts AS (
    ...
    SELECT
      ...
      g.medoid,
      CASE
        WHEN g.refinado THEN 'v6-global-complete-pairseed-refine-max' || p_max_satelites::text
        ELSE 'v6-global-complete-pairseed-max' || p_max_satelites::text
      END,
      g.threshold_usado,
      'pendente'
    FROM grupos g
...
COMMENT ON FUNCTION public.cluster_tickets_equipe(uuid, real, integer, integer, integer) IS
  'Agrupa tickets livres com recall global e complete-link por pair-seed. Desde v6, a subcategoria deixa de ser trava dura; o planeta final so aceita pares com coesao total acima do threshold.';
